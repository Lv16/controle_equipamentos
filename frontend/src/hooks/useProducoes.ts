...
const mapApiToProducao = (producao: any): Producao => ({
  id: producao.id,
...
const mapProducaoToApi = (producao: CreateProducaoDto | Producao) => ({
...
export const useProducoes = () => {
  ...
  const criarProducao = async (novaProducao: CreateProducaoDto) => {
    ...
    setProducoes((prev) => [producaoCriada, ...prev]);
    return producaoCriada;
  };
